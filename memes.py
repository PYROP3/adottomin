...
def draw_text_with_bbox(text: str, font_family: str, center_anchor: tuple, bbox: tuple, draw_ctx: ImageDraw.Draw, img, fill: tuple[int, int, int, int]=(0,0,0,255)):
    ...
    if use_pilmoji and use_emoji:
        ...
        with Pilmoji(img) as pilmoji:
            pilmoji.text(_anchor, get_wrapped_text(text, fnt, bbox[0], False), font=fnt, anchor="mm", fill=fill, align=_align, emoji_position_offset=(int(-_bbox[2]/4), int(-_bbox[3]/4)))
    else:
        draw_ctx.text(center_anchor, get_wrapped_text(text, fnt, bbox[0], True), font=fnt, anchor="mm", fill=fill, align=_align)
...
_baserules = {
    ...
    "text": lambda image, draw, args: draw_text_with_bbox(args["text"], "arial.ttf", args["position"], args["bbox"], draw, image, fill=args["fill"])
}

def _args_icon(icon: str, size: tuple[int, int], position: tuple[int, int]):
    return ("icon", {"icon": icon, "size": size, "position": position})
    
def _args_text(text: str, bbox: tuple[int, int], position: tuple[int, int], fill: tuple[int, int, int, int]=(0,0,0,255)):
    return ("text", {"text": text, "bbox": bbox, "position": position, "fill": fill})
...
def _args_for(id: str, author_icon: str=None, icon: str=None, text: str=None):
    ...
    if id == "walt": return [_args_text(text, (300, 300), (177, 202), fill=(255,255,255,255))]
